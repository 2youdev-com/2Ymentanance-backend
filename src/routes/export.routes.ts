  ...
  try {
    ...
    const logs = await prisma.maintenanceLog.findMany({
      ...
      include: {
        ...
        checklistItems: true,
        problemReport: {
          select: { category: true, severity: true, resolved: true },
        },
      },
    });
    ...
    const rows = [
      ...
      ...logs.map((log) => {
        ...
        return [
          ...
          log.type || '',
          log.status || '',
          log.technician?.fullName || log.technician?.username || '',
    ...
    console.error('EXPORT MAINTENANCE ERROR:', err);
    next(err);
  }
});
  ...
  try {
    ...
    const rows = [
      ...
      ...assets.map((a) => [
        ...
        a._count?.maintenanceLogs || 0,
        a.creator?.fullName || '',
        a.createdAt ? a.createdAt.toISOString().split('T')[0] : '',
        `"${a.remarks || ''}"`,
      ].join(',')),
    ].join('\n');
